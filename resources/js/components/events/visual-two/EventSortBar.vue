...
<template>
    <div class="flex flex-wrap items-center justify-end gap-2">
        <label for="sort-by-cal" class="flex items-center gap-1.5 text-sm text-muted-foreground">
        ...
        <select
            ...
            @change="emit('change')"
        >
            <option v-for="option in VISUAL_TWO_SORT_OPTIONS" :key="option.value" :value="option.value">
                {{ option.label }}
            </option>
        </select>
    </div>
